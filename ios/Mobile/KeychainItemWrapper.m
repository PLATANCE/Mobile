                 ...
                 rejecter : (RCTPromiseRejectBlock)reject)
{
    ...
    KeychainItemWrapper *keychainWrapper = [[KeychainItemWrapper alloc] initWithIdentifier:@"plating_uuid" accessGroup: nil];
    NSString *uuidValue = [keychainWrapper objectForKey:kSecAttrAccount];
    if(uuidValue == nil || uuidValue.length <= 0) {
        CFUUIDRef uuidRef = CFUUIDCreate(NULL);
        CFStringRef uuidStringRef = CFUUIDCreateString(NULL, uuidRef);
        CFRelease(uuidRef);

        uuidValue = [NSString stringWithString:(__bridge NSString *)uuidStringRef];
